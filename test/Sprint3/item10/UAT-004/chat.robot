...
...               ฟีเจอร์แชททั้งหมด: ส่งข้อความ, Quick Reply, Widget UI,
...
*** Variables ***
${PASSENGER_EMAIL}             passenger123@test.com
${DRIVER_EMAIL}                driver123@test.com
${PASSWORD}                    00000000
...
Driver Receives Message And Replies
    ...
    [Setup]            Setup Test Session    ${DRIVER_EMAIL}    ${PASSWORD}
    Open Chat History Page
    Open Chat Room From History    ${CONFIRMED_BOOKING_ID}
...
# ══════════════════════════════════════════════════════════════════
# 11) SECURITY WARNING — ข้อความที่มี sensitive keyword แสดง warning
# ══════════════════════════════════════════════════════════════════
Passenger Sees Security Warning When Sending Sensitive Keyword
    [Documentation]    ผู้โดยสารพิมพ์ข้อความที่มีคำ sensitive (เช่น "เบอร์โทร") → แถบแจ้งเตือนสีส้มปรากฏ
    [Tags]             passenger    chat    security
    [Setup]            Setup Test Session    ${PASSENGER_EMAIL}    ${PASSWORD}
    Go To Booking Page    ${CONFIRMED_BOOKING_ID}
    Open Chat Widget    ${CONFIRMED_BOOKING_ID}
    Send Message    เบอร์โทรของฉันคือ 089-123-4567
    Verify Security Warning Is Visible
    [Teardown]         Teardown Test Session

Security Warning Appears For Phone Number Pattern
    [Documentation]    พิมพ์เบอร์โทรขึ้นต้นด้วย 08 → แจ้งเตือนความปลอดภัยปรากฏ
    [Tags]             passenger    chat    security
    [Setup]            Setup Test Session    ${PASSENGER_EMAIL}    ${PASSWORD}
    Go To Booking Page    ${CONFIRMED_BOOKING_ID}
    Open Chat Widget    ${CONFIRMED_BOOKING_ID}
    Send Message    ติดต่อฉันได้ที่ 0891234567
    Verify Security Warning Is Visible
    [Teardown]         Teardown Test Session

Security Warning Appears For Address ID Keyword
    [Documentation]    พิมพ์ "ที่อยู่" → แจ้งเตือนความปลอดภัยปรากฏ
    [Tags]             passenger    chat    security
    [Setup]            Setup Test Session    ${PASSENGER_EMAIL}    ${PASSWORD}
    Go To Booking Page    ${CONFIRMED_BOOKING_ID}
    Open Chat Widget    ${CONFIRMED_BOOKING_ID}
    Send Message    ขอที่อยู่
    Verify Security Warning Is Visible
    [Teardown]         Teardown Test Session

Security Warning Does Not Appear For Normal Text
    [Documentation]    ส่งข้อความทั่วไปที่ไม่มี sensitive keyword → แถบแจ้งเตือนไม่ปรากฏ
    [Tags]             passenger    chat    security
    [Setup]            Setup Test Session    ${PASSENGER_EMAIL}    ${PASSWORD}
    Go To Booking Page    ${CONFIRMED_BOOKING_ID}
    Open Chat Widget    ${CONFIRMED_BOOKING_ID}
    Send Message    สวัสดีครับ ขอบคุณมากครับ
    Verify Security Warning Is Not Visible
    [Teardown]         Teardown Test Session

Security Warning Appears When Location Is Shared
    [Documentation]    กดส่งตำแหน่ง → แถบแจ้งเตือนความปลอดภัยปรากฏหลังส่ง
    [Tags]             passenger    chat    security    attachment
    [Setup]            Setup Test Session    ${PASSENGER_EMAIL}    ${PASSWORD}
    Go To Booking Page    ${CONFIRMED_BOOKING_ID}
    Open Chat Widget    ${CONFIRMED_BOOKING_ID}
    Open Attachment Menu
    Verify Attachment Menu Has Location Option
    Close Attachment Menu
    # หมายเหตุ: การกด "ส่งตำแหน่ง" จริงต้องได้รับอนุญาต Geolocation จากเบราว์เซอร์
    # ในสภาพแวดล้อม Robot Framework ควร Mock หรือตรวจสอบ UI ว่า Confirm dialog ปรากฏขึ้น
    [Teardown]         Teardown Test Session

# ══════════════════════════════════════════════════════════════════
# 12) READ STATUS — ตรวจสอบสถานะการอ่านข้อความ
# ══════════════════════════════════════════════════════════════════
Read Status Shows Single Checkmark For Sent Message
    [Documentation]    ผู้โดยสารส่งข้อความ → เห็น Checkmark เดียว (ส่งสำเร็จ แต่ยังไม่ถูกอ่าน)
    [Tags]             passenger    chat    read-status
    [Setup]            Setup Test Session    ${PASSENGER_EMAIL}    ${PASSWORD}
    Go To Booking Page    ${CONFIRMED_BOOKING_ID}
    Open Chat Widget    ${CONFIRMED_BOOKING_ID}
    Send Message    UAT-read-status-ทดสอบ
    Scroll Chat To Bottom
    # ตรวจว่า checkmark (svg) ปรากฏถัดจากเวลาส่ง
    Verify Read Status Indicator Exists
    [Teardown]         Teardown Test Session

Unread Badge Appears When Passenger Has Unread Messages
    [Documentation]    คนขับส่งข้อความ → ผู้โดยสารที่ยังไม่เปิดดูจะเห็น badge unread บน navbar
    [Tags]             passenger    driver    chat    read-status    notification
    [Setup]            Setup Test Session    ${DRIVER_EMAIL}    ${PASSWORD}
    Open Chat History Page
    Open Chat Room From History    ${CONFIRMED_BOOKING_ID}
    Send Message    UAT-unread-badge-ทดสอบจากคนขับ
    Scroll Chat To Bottom
    [Teardown]         Teardown Test Session

# ══════════════════════════════════════════════════════════════════
# 13) MESSAGE DELETION — ตรวจสอบการลบข้อความใน UI
# ══════════════════════════════════════════════════════════════════
Passenger Can Delete Own Message
    [Documentation]    ผู้โดยสาร Hover ที่ข้อความตัวเองแล้วกดปุ่มถังขยะ → ข้อความแสดงว่าถูกลบ
    [Tags]             passenger    chat    delete-message
    [Setup]            Setup Test Session    ${PASSENGER_EMAIL}    ${PASSWORD}
    Go To Booking Page    ${CONFIRMED_BOOKING_ID}
    Open Chat Widget    ${CONFIRMED_BOOKING_ID}
    Send Message    UAT-to-delete-ข้อความทีจะลบ
    Scroll Chat To Bottom
    Hover Over Last Own Message And Click Delete
    Accept Deletion Confirmation
    Verify Page Contains Deleted Message Placeholder
    [Teardown]         Teardown Test Session

Delete Button Not Visible Without Hover
    [Documentation]    ปุ่มลบไม่ปรากฏ จะเห็นได้เฉพาะเมื่อ Hover
    [Tags]             passenger    chat    delete-message    ui
    [Setup]            Setup Test Session    ${PASSENGER_EMAIL}    ${PASSWORD}
    Go To Booking Page    ${CONFIRMED_BOOKING_ID}
    Open Chat Widget    ${CONFIRMED_BOOKING_ID}
    Scroll Chat To Bottom
    Verify Delete Button Is Hidden Without Hover
    [Teardown]         Teardown Test Session

Deleted Message Shows Placeholder To Driver
    [Documentation]    เมื่อผู้โดยสารลบข้อความ คนขับจะเห็น "ข้อความนี้ถูกลบแล้ว" แทนข้อความเดิม
    [Tags]             driver    passenger    chat    delete-message
    [Setup]            Setup Test Session    ${DRIVER_EMAIL}    ${PASSWORD}
    Open Chat History Page
    Open Chat Room From History    ${CONFIRMED_BOOKING_ID}
    Scroll Chat To Bottom
...
Open Chat Room For Booking
    ...
    Sleep    2s
    # Use JS click to ensure the room is opened even if obstructed
    Execute JavaScript    document.querySelector("div[data-booking-id='${booking_id}']").click()
    Wait Until Element Is Visible    id:chat-container    timeout=15s
...
Minimize Chat Widget
    # Use JS click to avoid ElementClickInterceptedException on the floating header
    Execute JavaScript    document.getElementById('chat-header-maximized').click()
    Wait Until Element Is Visible    id:chat-widget-minimized    timeout=5s

Maximize Chat Widget
    # Use JS click to avoid ElementClickInterceptedException on the minimized widget
    Execute JavaScript    document.getElementById('chat-widget-minimized').click()
...
Click Quick Reply
    ...
    # Use JS click to avoid ElementClickInterceptedException on quick reply buttons
    Execute JavaScript    document.evaluate("//button[contains(text(), '${reply_text}')]", document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue.click()
    Sleep    1s
...
# ══════════════════════════════════════════════════════════════════
# SECURITY WARNING KEYWORDS
# ══════════════════════════════════════════════════════════════════
Verify Security Warning Is Visible
    [Documentation]    ตรวจสอบว่าแถบแจ้งเตือนความปลอดภัยสีส้มปรากฏในแชท
    Scroll Chat To Bottom
    Wait Until Page Contains Element    xpath://p[contains(text(), 'โปรดระวังการส่งข้อมูลส่วนตัว')]    timeout=8s
    Element Should Be Visible    xpath://p[contains(text(), 'โปรดระวังการส่งข้อมูลส่วนตัว')]
    Log    Security warning banner is visible

Verify Security Warning Is Not Visible
    [Documentation]    ตรวจสอบว่าแถบแจ้งเตือนความปลอดภัยไม่ปรากฏ (ข้อความทั่วไป)
    Scroll Chat To Bottom
    Sleep    1s
    ${present}=    Run Keyword And Return Status    Page Should Not Contain    โปรดระวังการส่งข้อมูลส่วนตัว
    IF    not ${present}
        Log    WARNING: Security banner unexpectedly found after normal message    level=WARN
    END
    Log    Security warning banner is not visible (expected)

# ══════════════════════════════════════════════════════════════════
# READ STATUS KEYWORDS
# ══════════════════════════════════════════════════════════════════
Verify Read Status Indicator Exists
    [Documentation]    ตรวจสอบว่าไอคอน Checkmark สถานะการอ่านปรากฏในแถวของข้อความล่าสุด
    # ตรวจสอบว่ามี svg ใน div ที่ justify-end (ข้อความของตัวเอง)
    Wait Until Page Contains Element
    ...    xpath://div[contains(@class,'justify-end')]//div[contains(@class,'flex') and .//*[local-name()='svg']]
    ...    timeout=10s
    Log    Read status indicator (checkmark) found for own message

Verify Room Unread Count Is Zero
    [Arguments]    ${booking_id}
    [Documentation]    ตรวจสอบว่า unread badge ของห้องแชทนี้หายไปหลังจากอ่านข้อความ
    ${badge_exists}=    Run Keyword And Return Status
    ...    Wait Until Page Contains Element    xpath://div[contains(@id, 'room-unread-badge-')]    timeout=5s
    IF    ${badge_exists}
        # ถ้ายังมี badge อยู่ → ตรวจว่าเป็น 0
        ${badge_text}=    Get Text    xpath:(//div[contains(@id, 'room-unread-badge-')])[1]
        Should Be Equal As Strings    ${badge_text}    0    Unread count should be 0 after reading
    ELSE
        Log    No unread badge found — room is fully read (OK)
    END

# ══════════════════════════════════════════════════════════════════
# MESSAGE DELETION KEYWORDS
# ══════════════════════════════════════════════════════════════════
Hover Over Last Own Message And Click Delete
    [Documentation]    ค้นหาข้อความล่าสุดที่ตัวเองส่ง → Hover ให้ปุ่มลบปรากฏ → กดปุ่ม
    Scroll Chat To Bottom
    # หา message bubble ล่าสุดที่อยู่ทางขวา (isMine = justify-end)
    Wait Until Page Contains Element
    ...    xpath:(//div[contains(@class,'justify-end') and not(contains(@class,'items-center'))])[last()]
    ...    timeout=10s
    ${last_msg}=    Get WebElement
    ...    xpath:(//div[contains(@class,'justify-end') and not(contains(@class,'items-center'))])[last()]
    Mouse Over    ${last_msg}
    Sleep    0.5s
    # กดปุ่มลบที่ปรากฏขึ้น (ปุ่มถังขยะ)
    Wait Until Element Is Visible
    ...    xpath:(//button[@title='ลบข้อความ'])[last()]    timeout=5s
    Click Element    xpath:(//button[@title='ลบข้อความ'])[last()]
    Log    Delete button clicked

Accept Deletion Confirmation
    [Documentation]    ยืนยัน Browser Confirm Dialog เพื่อยืนยันการลบข้อความ
    Handle Alert    accept
    Sleep    1s
    Log    Deletion confirmed via dialog

Verify Page Contains Deleted Message Placeholder
    [Documentation]    ตรวจสอบว่าข้อความล่าสุดแสดง placeholder "ข้อความนี้ถูกลบแล้ว"
    Scroll Chat To Bottom
    Wait Until Page Contains    ข้อความนี้ถูกลบแล้ว    timeout=10s
    Log    Deleted placeholder text is visible

Verify Delete Button Is Hidden Without Hover
    [Documentation]    ตรวจสอบว่าปุ่มลบไม่ปรากฏขึ้นเมื่อยังไม่ได้ Hover
    ${visible}=    Run Keyword And Return Status
    ...    Element Should Be Visible    xpath://button[@title='ลบข้อความ']
    IF    ${visible}
        Log    WARNING: Delete button is unexpectedly visible without hover    level=WARN
    ELSE
        Log    Delete button is correctly hidden without hover (opacity-0)
    END
