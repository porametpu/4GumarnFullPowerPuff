...
const { getDirections, geocode, reverseGeocode, suggest, search } = require('../utils/googleMaps');
...
const suggestCtrl = asyncHandler(async (req, res) => {
    const { keyword } = req.query;
    if (!keyword)
        throw new ApiError(400, 'keyword is required');
    const data = await suggest(keyword);
...
module.exports = { directions, geocodeCtrl, reverseGeocodeCtrl, suggestCtrl, searchCtrl };
